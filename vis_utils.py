...
def visualize_ds(ds, num_images, rows, cmap = None, class_names:list = None, data_type = None, save_folder=None):
    ...
    print(f"Examples from {data_type} dataset loaded to {save_folder} folder...")
    print("---------------------------------------------------------------------")
    #plt.show()
    plt.clf(); plt.close()
...
def visualize_metrics(train_acc_list, val_acc_list, train_loss_list, val_loss_list, save_dir): 
    ...
    print(f"Training and Validation accuracy metrics downloaded to {save_dir} folder...")
    #plt.show()
    plt.close()
    ...
    print(f" Training and Validation loss metrics downloaded to {save_dir} folder...\n")
   # plt.show()
...
def visualize_inference(rasmlar_list, haqiqiy_list, bashorat_list,  class_names, save_dir):
    ...
    print(f"Inference results downloaded to {save_dir} folder...")
...
def visualize_gradcam(model, img_list, gt_list, pred_list, class_names, num_imgs, row, img_size, save_dir): 
    ...
    for i, index in enumerate(indexs):
        ...
        resized_heatmap = cv2.resize(heatmap, (img_size), interpolation=cv2.INTER_AREA)
    ...
    print(f"GradCam inference results saved to {save_dir} folder ...\n")
